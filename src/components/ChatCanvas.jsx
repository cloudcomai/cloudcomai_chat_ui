...
export default function ChatCanvas({
  ...
  composer, setComposer, onSendMessage, apiBridge
}) {
  ...
  const handleCastVote = async (pollId, optionId) => {
    if (!apiBridge) return;
    try {
      await apiBridge('/polls.php?action=vote', {
        method: 'POST',
        ...
        <div className="canvas-action-utilities">
          ...
          {selectedChat?.type === 'group' || selectedChat?.isGroup ? (
            <>
              ...
              <button className="action-utility-btn text-red" style={{color:'#ef4444'}} onClick={() => setModal('manage_members')}><X size={16}/><span>Manage/Remove</span></button>
            </>
        ...
        {messages.map(msg => {
          ...
          return (
            <div key={msg.id} className={`message-bubble-wrapper ${isMine ? 'outgoing-align' : 'incoming-align'}`}>
              {isPoll ? (
                <div className="poll-bubble-card" style={{background:'var(--bg-primary)',border:'1px solid var(--border-color)',borderRadius:'16px',padding:'16px',minWidth:'280px',maxWidth:'70%',boxShadow:'0 4px 6px -1px rgba(0,0,0,0.05)',marginBottom:'4px'}}>
                  <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'12px'}}><span style={{fontSize:'18px'}}>📊</span><h4 style={{fontSize:'14px',fontWeight:'700',color:'var(--text-main)',margin:0}}>{messageContent.replace('📊 POLL: ','')}</h4></div>
                  <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
                    <button type="button" onClick={() => handleCastVote(msg.poll_id, msg.option_a_id)} style={{position:'relative',background:'var(--bg-directory)',padding:'10px 12px',borderRadius:'8px',border:'1px solid var(--border-color)',fontSize:'13px',cursor:'pointer',overflow:'hidden',textAlign:'left'}}><span style={{position:'relative'}}>Option Choice A</span></button>
                    <button type="button" onClick={() => handleCastVote(msg.poll_id, msg.option_b_id)} style={{position:'relative',background:'var(--bg-directory)',padding:'10px 12px',borderRadius:'8px',border:'1px solid var(--border-color)',fontSize:'13px',cursor:'pointer',overflow:'hidden',textAlign:'left'}}><span style={{position:'relative'}}>Option Choice B</span></button>
                  </div>
                  <div className="bubble-meta-footer" style={{marginTop:'10px',display:'flex',justifyContent:'space-between',fontSize:'10px',color:'var(--text-light')}}><span>Active Voting Room</span><span>{msg.time || 'Just Now'}</span></div>
                </div>
              ) : (
