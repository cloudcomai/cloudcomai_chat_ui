...
import {
  ...
  Reply, Edit3, Plus, Camera, Pin, X, Send
} from 'lucide-react';
    ...
    <main className="chat-interaction-canvas">
      <header className="canvas-header-nav">
        {selectedChat ? (
          ...
          <div className="active-interlocutor-card">
            <h4>Select a conversation to begin</h4>
          </div>
        )}

        <div className="canvas-action-utilities">
          <button className="action-utility-btn" onClick={() => setModal('audio')}>
            <Phone size={18} />
            <span>Audio Call</span>
          </button>
          <button className="action-utility-btn" onClick={() => setModal('video')}>
            <Video size={18} />
            <span>Video Call</span>
          </button>
          <button className="action-utility-btn" onClick={() => setModal('poll')}>
            <BarChart3 size={18} />
            <span>New Poll</span>
          </button>
          <button className="action-utility-btn" onClick={() => setModal('location')}>
            <MapPin size={18} />
          ...
          {selectedChat?.type === 'group' || selectedChat?.isGroup ? (
            <>
              ...
              <button
                className="action-utility-btn text-red"
                style={{ color: '#ef4444' }}
                onClick={() => setModal('manage_members')}
              >
                <X size={16} />
            ...
            <button className="action-utility-btn" onClick={() => setModal('group')}>
              <Users size={18} />
              <span>New Group</span>
            </button>
          )}
          ...
          <button className="icon-utility-only">
            <Search size={18} />
          </button>
          <button className="icon-utility-only">
      ...
      <div
        className="message-history-viewport"
        ref={historyRef}
        onScroll={handleHistoryScroll}
      >
        <div className="encryption-note">
          Messages are protected in transit. E2EE configuration applied.
        </div>
          ...
          return (
            <div
              ...
              className={`message-bubble-wrapper ${
                isMine ? 'outgoing-align' : 'incoming-align'
              }`}
            >
              {isPoll ? (
                <div className="poll-bubble-card" style={pollCardStyle}>
                  ...
                  <div style={pollOptionsStyle}>
                    <button
                      type="button"
                      onClick={() =>
                        handleCastVote(msg.poll_id, msg.option_a_id)
                      }
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        handleCastVote(msg.poll_id, msg.option_b_id)
                      }
                      style={pollOptionStyle}
                    >
                ...
                <div
                  className={`message-data-bubble ${
                    isMine ? 'primary-accent' : 'neutral-fallback'
                  }`}
                >
                  {msg.reply_to_text && (
                    <div className="reply-preview-context">
                      <Reply size={12} /> {msg.reply_to_text}
                    </div>
                  )}
      ...
      <div className="canvas-bottom-action-tray">
        <div className="shortcut-action-grid">
          <button
            className="shortcut-action-card green-theme"
          ...
          <button
            className="shortcut-action-card purple-theme"
          ...
          <button
            className="shortcut-action-card yellow-theme"
          ...
          <button
            className="shortcut-action-card teal-theme"
        ...
        {replyTo || editing ? (
          <div className="context-bar">
            <div>
              ...
              <strong>
                {(editing || replyTo).body || (editing || replyTo).text}
              </strong>
            </div>
            <button
              onClick={() => {
        ...
        <div className="message-input-composer-bar">
          ...
          <button className="composer-addon-btn">
            <Pin size={18} />
          </button>
          <button className="composer-addon-btn">
            <Camera size={18} />
          ...
          <button
            className="voice-mic-submit-btn"
