  ...
  const executeMemberAction = async (targetUserId, actionType) => {
    ...
    try {
      await apiBridge('/group_members.php', {
        method: 'POST',
        body: JSON.stringify({ chat_id: selectedChat.id, user_id: targetUserId, action: actionType })
      });
    ...
    <div className="modal-backdrop">
      <div className="modal-content-card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          <button onClick={close} style={{ color: 'var(--text-light)', background: 'none', border: 'none' }}><X size={20}/></button>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ maxHeight: '200px', overflowY: 'auto', marginTop: '8px' }}>
              {!hasSearched && searchResults.length === 0 && (
                <p style={{ fontSize: '12px', color: 'var(--text-light)', textAlign: 'center', padding: '12px' }}>Type a name or email address above to begin searching.</p>
              )}
              {hasSearched && searchResults.length === 0 && !loading && (
                <div style={{ textAlign: 'center', padding: '20px 12px' }}>
                  <p style={{ fontSize: '13px', fontWeight: '600', color: '#ef4444', marginBottom: '4px' }}>🔍 No Users Found</p>
                  <p style={{ fontSize: '11px', color: 'var(--text-light)' }}>We couldn't find any accounts matching "{searchTerm}". Check the spelling and try again.</p>
                </div>
              )}
              {searchResults.length > 0 && searchResults.map(u => (
                <div key={u.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px', borderBottom: '1px solid var(--border-color)' }}>
                  <div><h5 style={{ fontSize: '14px', fontWeight: '600', color: 'var(--text-main)' }}>{u.name}</h5><span style={{ fontSize: '11px', color: 'var(--text-light)' }}>@{u.username || u.user_id}</span></div>
                  <button style={{ color: '#10b981', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '12px', fontWeight: '500', background: 'none', border: 'none' }} onClick={() => executeMemberAction(u.id, 'add')} disabled={loading}>+ Add</button>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <div style={{ maxHeight: '240px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {!currentMembers || currentMembers.length === 0 ? (
                ...
                return (
                  <div key={m.id || m.user_id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: 'var(--bg-directory)', borderRadius: '8px', marginBottom: '4px' }}>
                    <div>
                    ...
                    {isOwner ? (
                      <span style={{ color: 'var(--text-light)', fontSize: '11px', fontWeight: '600' }}>Owner</span>
                    ) : (
