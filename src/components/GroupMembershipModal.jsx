...
export default function GroupMembershipModal({ type, selectedChat, apiBridge, close, onActionComplete }) {
  ...
  const [hasSearched, setHasSearched] = useState(false); // 👈 Added missing state tracker variable

  // Fetch current group members when opening the manage overlay
  useEffect(() => {
  ...
  const handleSearchUsers = async () => {
    ...
    setLoading(true);
    setHasSearched(true); // 👈 Toggles true right when search completes execution
    try {
  ...
  // Dispatch Membership Modification Actions
    ...
    try {
      await apiBridge('/group_members.php', {
        ...
        body: JSON.stringify({
          ...
          action: actionType // 'add' or 'remove'
        })
      });
      alert(`User successfully ${actionType === 'add' ? 'added to' : 'removed from'} group.`);
      
      // Reset states on success to force clean refresh metrics
      setSearchTerm('');
      setSearchResults([]);
  ...
  return (
    <div className="modal-backdrop">
      <div className="modal-content-card">
        ...
        {/* MODAL TITLE HEADER CONTAINER */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}> {/* 👈 Fixed alignLive typo */}
        ...
        {/* ==========================================================================
           CONTAINER A: USER SEARCH BOX TO ADD MEMBERS
           ========================================================================== */}
        {type === 'add_member' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ maxHeight: '200px', overflowY: 'auto', marginTop: '8px' }}>
              {/* 1. INITIAL STATE: User hasn't typed anything yet */}
              ...
              {/* 2. NO RESULTS STATE: The search ran but the database returned 0 rows */}
              ...
              {/* 3. ACTIVE DATA STATE: Render matching profiles seamlessly */}
        ...
        {/* ==========================================================================
           CONTAINER B: ACTIVE MEMBERS WITH REMOVE ACTION TRIGGERS
           ========================================================================== */}
        {type === 'manage_members' && (
