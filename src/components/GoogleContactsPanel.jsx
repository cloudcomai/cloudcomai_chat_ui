  ...
  return (
    <div className="modal-backdrop">
      <div className="modal-content-card" style={{ width: 'min(620px, 100%)', maxHeight: '88vh', overflowY: 'auto' }}>
        ...
        {error && (
          <div style={{ padding: '10px 12px', marginBottom: '12px', borderRadius: '9px', background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', fontSize: '12px' }}>
            {error}
          </div>
        )}
        ...
        {loading ? (
          ...
          <div style={{ border: '1px solid var(--border-color)', borderRadius: '12px', padding: '22px', textAlign: 'center' }}>
            <div style={{ width: 48, height: 48, borderRadius: '12px', margin: '0 auto 12px', display: 'grid', placeItems: 'center', background: 'var(--bg-directory)', color: 'var(--primary-color)' }}>
              <Contact size={24} />
            </div>
            <strong style={{ display: 'block', marginBottom: '6px' }}>Connect your Google Contacts</strong>
            <p style={{ margin: '0 auto 16px', maxWidth: 430, fontSize: '12px', lineHeight: 1.5, color: 'var(--text-muted)' }}>
              CloudComAI will only request read-only access to your Google Contacts. Your Gmail messages are not requested.
            </p>
            <button type="button" onClick={connectGoogle} disabled={connecting} style={primaryButtonStyle}>
          ...
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px', border: '1px solid var(--border-color)', borderRadius: '12px', marginBottom: '14px' }}>
              <div style={{ width: 38, height: 38, borderRadius: '50%', display: 'grid', placeItems: 'center', background: 'var(--bg-directory)', color: 'var(--primary-color)' }}>
                <Check size={19} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <strong style={{ display: 'block' }}>Google connected</strong>
                <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{status.email || 'Google account'}</span>
              </div>
              <button type="button" onClick={syncContacts} disabled={syncing} style={secondaryButtonStyle}>
            ...
            <div style={{ border: '1px solid var(--border-color)', borderRadius: '12px', overflow: 'hidden' }}>
              <div style={{ padding: '11px 13px', background: 'var(--bg-directory)', fontWeight: 700, fontSize: '13px' }}>Recent contacts</div>
              {contacts.length === 0 ? (
        ...
        <div style={{ marginTop: '14px', display: 'flex', justifyContent: 'flex-end' }}>
          <button type="button" onClick={close} style={secondaryButtonStyle}>Close <ChevronRight size={15} /></button>
        </div>
      </div>
    </div>
  );
...
const primaryButtonStyle = {
  border: 'none', borderRadius: '9px', padding: '10px 16px', background: 'var(--primary-color)', color: '#fff', fontWeight: 600, cursor: 'pointer'
};

const secondaryButtonStyle = {
  display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '8px 10px', background: 'var(--bg-primary)', color: 'var(--text-main)', fontSize: '12px', fontWeight: 600, cursor: 'pointer'
};

const statCardStyle = {
  display: 'flex', flexDirection: 'column', gap: '4px', padding: '11px 12px', border: '1px solid var(--border-color)', borderRadius: '10px', background: 'var(--bg-primary)'
};
